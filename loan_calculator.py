...
class LoanEligibilityCalculator:
    ...
    def calculate_emi(self, loan_amount, interest_rate, years):
    ...
    def calculate_eligibility(self, applicant_data, salary_slips, emis_list, 
        ...
        #  Determine final tenure
        if requested_tenure_years:
            tenure_years = requested_tenure_years
            if tenure_years > remaining_years:
                 result['warnings'].append(
            f"Requested tenure ({tenure_years} yrs) exceeds remaining service life ({remaining_years} yrs)."
        )
        else:
            tenure_years = min(remaining_years, self.MAX_TENURE_YEARS)
        result['calculations']['final_tenure_years'] = tenure_years
        # Calculate EMI
        emi = self.calculate_emi(requested_loan_amount, interest_rate, tenure_years)
        result['calculations']['emi'] = emi
         # Compute income details
        gross_income, _, _, _ = self.calculate_gross_monthly_income(salary_slips)
        total_obligations, _ = self.calculate_total_obligations(emis_list, excluded_emis)
        # FOIR
        foir = self.calculate_foir(total_obligations + emi, gross_income)
        result['calculations']['foir_percent'] = foir
        #  Final eligibility
        if foir <= (self.FOIR_SALARIED * 100):
               result['eligible'] = True
        else:
               result['eligible'] = False
               result['issues'].append(
         f"FOIR too high: {foir:.2f}% > allowed {self.FOIR_SALARIED * 100}%"
    )
