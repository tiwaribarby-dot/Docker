    ...
    with st.sidebar:
        ...
        with st.expander("📋 View/Edit Loan Policy", expanded=False):
          ...
          step=100000
            ...
            value=8.5,
            step=0.1,
            help="Enter annual interest rate"
            )
            #  NEW — Loan Tenure Input
            loan_tenure_years = st.number_input(
                 "Loan Tenure (Years)",
    min_value=1,
    max_value=st.session_state.loan_config['max_tenure_years'],
    value=st.session_state.loan_config['max_tenure_years'],
    step=1,
    help="Choose lesser tenure if customer does not want full max term"
            )
    ...
    with tab1:
        ...
        if st.button("🔍 Analyze All Documents", type="primary", use_container_width=True):
            if not os.environ.get("OPENAI_API_KEY"):
                ...
                analyze_all_documents(uploaded_files, requested_amount, interest_rate, loan_tenure_years)
...
def analyze_all_documents(files, loan_amount, interest_rate, loan_tenure_years):
    """Main analysis function with auto document categorization"""

    # ✅ Ensure analyzed_data exists
    if "analyzed_data" not in st.session_state or st.session_state.analyzed_data is None:
        st.session_state.analyzed_data = {}

    # ✅ Store loan inputs
    st.session_state.analyzed_data["loan_amount"] = loan_amount
    st.session_state.analyzed_data["interest_rate"] = interest_rate
    st.session_state.analyzed_data["loan_tenure_years"] = loan_tenure_years
    ...
    with st.spinner("🤖 AI is analyzing documents with GPT-5... This may take a few minutes..."):
        try:
            ...
            # ✅ Define base dictionaries
            all_analyzed = {}   # FIXED — now properly defined
            categorized = {
                "salary_slips": [],
                "bank_statements": [],
                "pan": None,
            ...
            for idx, file in enumerate(files):
                ...
                if doc_type == 'salary_slip':
                    ...
                    if 'error' not in result:
                        ...
                        applicant_info.setdefault('employer', result.get('employer', 'N/A'))
                        applicant_info.setdefault('designation', result.get('designation', 'N/A'))
                        applicant_info.setdefault('department', result.get('department', 'N/A'))
                    ...
                    if 'error' not in result:
                        ...
                        applicant_info.setdefault('applicant_name', result.get('name', 'N/A'))
            ...
            all_analyzed.update({
                'salary_slips': categorized['salary_slips'],
                'pan': categorized['pan'],
                'aadhar': categorized['aadhar'],
                'form16': categorized['form16']
            })
            ...
            if applicant_info.get('date_of_birth'):
                applicant_info['current_age'] = loan_calc.calculate_age(applicant_info['date_of_birth'])
            ...
            for key, value in form_data.items():
                applicant_info.setdefault(key, value)
            ...
            queries = query_gen.generate_queries(analysis_summary, eligibility, pending_docs, pending_forms)
            formatted_queries = query_gen.format_queries_for_report(queries)

            status_text.text("✅ Analysis complete!")
            progress_bar.progress(100)

            # ✅ Final analyzed data structure
            st.session_state.analyzed_data = {
                'applicant_info': applicant_info,
                'salary_analysis': {'salary_slips': categorized['salary_slips']},  # required for display_results()
...
def display_results(data):
    """Display comprehensive analysis results (stable version)"""
    import pandas as pd
    st.header("📊 Analysis Results")

    # ✅ Safe access for eligibility data
    eligibility_data = data.get("eligibility", {}) or {}
    calculations = eligibility_data.get("calculations", {}) or {}
    applicant_info = data.get("applicant_info", {}) or {}
    ...
    with col1:
        st.metric(
            ...
            f"₹{calculations.get('gross_monthly_income', 0):,.0f}"
        )

    with col2:
        foir = calculations.get('foir_with_requested_loan', 0.0)
        foir_limit = st.session_state.loan_config.get('foir_salaried', 60)
        delta_value = foir - foir_limit

        st.metric(
            "FOIR",
            f"{foir:.1f}%",
            delta=f"{delta_value:.1f}%" if delta_value > 0 else "Within Limit",
            delta_color="inverse"
        )
    ...
    with col4:
        tenure = calculations.get('approved_tenure_years', 'N/A')
        st.metric(
            "Tenure (Auto)",
    ...
    with st.expander("👤 APPLICANT DETAILS", expanded=True):
        col1, col2 = st.columns(2)
        with col1:
            st.write(f"**Name:** {applicant_info.get('applicant_name', 'N/A')}")
            st.write(f"**PAN:** {applicant_info.get('pan_masked', 'N/A')}")
            st.write(f"**Aadhar:** {applicant_info.get('aadhar_masked', 'N/A')}")
            st.write(f"**Age:** {applicant_info.get('current_age', 'N/A')} years")

        with col2:
            st.write(f"**Employer:** {applicant_info.get('employer', 'N/A')}")
            st.write(f"**Designation:** {applicant_info.get('designation', 'N/A')}")
            st.write(f"**Department:** {applicant_info.get('department', 'N/A')}")
            st.write(f"**Loan Amount:** ₹{applicant_info.get('loan_amount', 0):,.0f}")

    # 💰 Salary Breakdown
    with st.expander("💰 SALARY BREAKDOWN", expanded=True):
        salary_analysis = data.get('salary_analysis', {}) or {}
        salary_slips = salary_analysis.get('salary_slips', []) or []
        ...
        if salary_slips:
            ...
            st.info("No salary slips found or recognized.")

    # ✅ Eligibility Section
    with st.expander("✅ ELIGIBILITY DETAILS", expanded=True):
        calc = eligibility_data.get('calculations', {}) or {}
        eligible = eligibility_data.get('eligible', False)
        issues = eligibility_data.get('issues', [])
